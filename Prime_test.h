...
double VOLE_prime_QASD(u64 n)
{
  ...
  std::thread t1([&]() 
  {
    ...
    std::cout<<"The total time of VOLE based on QA-SD code consume "<<double(end - start)<<" seconds"<<std::endl;
...
double Vole_prime_LPN(u64 n, MultType type, bool debug, bool doFakeBase, bool mal)
{
    using VecF = typename Ctx::template Vec<F>;
...
template<typename F, typename Ctx>
double EA_prime_encode_test(u64 k, u64 R, u64 bw)
    {
...
double QA_prime_encode_test(int n)
{
    ...
    for(u64 i=0;i<n;i++)
    {
        mctx.fromBlock(a[i],prng.get());
        mctx.fromBlock(coeff[i],prng.get());
    }
    double start=omp_get_wtime();
    wht<F, CoeffCtx>(coeff,n);
    wht<F, CoeffCtx>(a,n);
    for(u64 i=0;i<n;i++)
    {
        mctx.mul(a[i], coeff[i], a[i]);
    }
    double end=omp_get_wtime();
    double total_time=double(end - start);
    std::cout<<"The time of QA Syndrome encoding consume "<<double(end - start)<<" seconds"<<std::endl;
    return total_time;
}
    ...
    template<typename F, typename CoeffCtx>
    double EC_prime_encode_test(u64 k, u64 r, u64 bw, u64 aw, bool sys)
    {
        u64 n=r*k;
        ExConvCode code;
