...
namespace osuCrypto
{
	struct RegularPrimeDpfKey
	{
		...
		bool operator==(const RegularPrimeDpfKey& o) const
		{
			return
				mSeed == o.mSeed &&
	...
	inline std::ostream& operator<<(std::ostream& o, const RegularPrimeDpfKey& k)
	{
		o << k.mSeed << std::endl;
	...
	inline void RegularPrimeDpf::init(
		u64 partyIdx,
		u64 domain,
		u64 numPoints)
	{
	...
	template<typename Output >
	macoro::task<> RegularPrimeDpf::expand(
		span<u64> points,
		span<u64> values,
		block seed,
		Output&& output,
	...
	inline macoro::task<> RegularPrimeDpf::keyGen(
		span<u64> points,
		span<u64> values,
		block seed,
		RegularPrimeDpfKey& outputKey,
	...
	//
	template<typename Output>
	macoro::task<> RegularPrimeDpf::implExpand(
		span<u64> points,
		span<u64> values,
		block seed,
		RegularPrimeDpfKey* inputKey,
...
#define SIMD8(VAR, STATEMENT) \
	...
	do{}while(0)
		...
		setBytes(diff, 0);
        std::vector<u64> tag_sum(mNumPoints);
		std::vector<u64> sign(mNumPoints);
		for(u64 i=0;i<mNumPoints;i++)
		{
			tag_sum[i]=0;
		}
			...
			for (u64 L = 0, L2 = 0; L2 < size; ++L, L2 += 2)
			{
				...
				for (u64 k = 0; k < numPoints8; k += 8)
				{
					for (u64 j = 0; j < 2; ++j)
					{
						...
						SIMD8(q, 
							tag[j][k + q] = tagBit(temp[q]); 
							tag_sum[k + q] += (lsb(tag[j][k + q]) % 2);
						);

						SIMD8(q, currentSeed[j][k + q] = AES::roundEnc(temp[q], temp[q]));
						if (mPartyIdx == 0)
						{
							SIMD8(q, ctx.bplus(diff[k + q], diff[k + q], currentSeed[j][k + q]));
						}
						else
						{
							SIMD8(q, ctx.bminus(diff[k + q], diff[k + q], currentSeed[j][k + q]));
						}
					}
				}
		...
		for (u64 k = numPoints8; k < mNumPoints; ++k)
		{
			for (u64 j = 0; j < 2; ++j)
			{
				...
				tag_sum[k] += (lsb(tag[j][k]) % 2); // 统一lsb函数提取最低位，替代_mm_extract_epi64
				auto rr = AES::roundEnc(temp[0], temp[0]);
				if (mPartyIdx == 0)
				{
					ctx.bplus(diff[k], diff[k], rr);
				}
				else
				{
					ctx.bminus(diff[k], diff[k], rr);
				}
				currentSeed[j][k] = rr;
			}
		}
            ...
            /*    
				for (u64 k = 0; k < mNumPoints; ++k)
				{
					for (u64 j = 0; j < 2; ++j)
					{
						temp[0] = currentSeed[j][k] ^ (parentTag[k] & sigma[j][k]);
						tag[j][k] = tagBit(temp[0]);
						tag_sum[k]=tag_sum[k]-(_mm_extract_epi64(tag[j][k],0)%2);
						auto rr  = AES::roundEnc(temp[0], temp[0]);
						if(mPartyIdx==0)
						{
						   ctx.bplus(diff[k],diff[k], rr);
		...
		if (values.size() || (inputKey && inputKey->mLeafVals.size()))
		{
			...
			{
				for (u64 k = 0; k < numPoints8; k=k+8)
				{
					SIMD8(q, ctx.mplus(diff[k+q],diff[k+q], values[k+q]));
					//std::cout<<tag_sum[k]<<"  "<< sign[k]<<std::endl;
					//gamma[k] ^= diff[k];
				}
				for (u64 k =numPoints8 ; k < mNumPoints; ++k)
				{
					//std::cout<<mPartyIdx<<"  "<<_mm_extract_epi64(diff[k],0)<<std::endl;
					ctx.mplus(diff[k], diff[k], values[k]);
					//diff[k] ^= values[k];

				}
				co_await sock.send(coproto::copy(diff));
				co_await sock.recv(gamma);
				co_await sock.send(coproto::copy(tag_sum));
				co_await sock.recv(sign);
				
				for (u64 k = 0; k < numPoints8; k=k+8)
				{
					SIMD8(q, ctx.bplus(gamma[k+q], gamma[k+q], diff[k+q]);sign[k+q]=(tag_sum[k+q]>sign[k+q])? 1: -1);
					//SIMD8(q, );
					//std::cout<<tag_sum[k]<<"  "<< sign[k]<<std::endl;
					//gamma[k] ^= diff[k];
				}
				for (u64 k = numPoints8; k < mNumPoints; k++)
				{
					ctx.bplus(gamma[k], gamma[k], diff[k]);
					sign[k]=(tag_sum[k]>sign[k])? 1: -1;
				}
			}
			...
			else
			{
				...
				for (u64 i = 0; i < mDomain; ++i)
				{
						...
						for (u64 k = 0; k < numPoints8; k += 8)
					{
						...
						block temp[8];
						SIMD8(q, T[q]=tdi[k+q]&gamma[k+q]);
						SIMD8(q, ctx.bminus(temp[q],zero,T[q]));
						SIMD8(q, T[q]=(sign[k+q]==1)? T[q]: temp[q]);
						if(mPartyIdx==0)
						{
							SIMD8(q, ctx.bminus(T[q], T[q],  sdi[k+q]));
						}
						else
						{
							SIMD8(q, ctx.bplus(T[q], T[q], sdi[k+q]));
						}
						
						SIMD8(q, output(k + q, i,  _mm_extract_epi64(T[q],0), tdi[k + q]));
					}
					/*
					for(u64 k=numPoints8;k<mNumPoints;k++)
					{
						u64 T= _mm_extract_epi64(tdi[k] & gamma[k],0);
						if(sign[k]==-1)
						{
							ctx.minus(T,0,T);
						}
						if(mPartyIdx==0)
						{
							ctx.minus(T, T,  _mm_extract_epi64(sdi[k],0));
						}
						else
						{
							ctx.plus(T, T, _mm_extract_epi64(sdi[k],0) );
						}
						output(k , i, T, tdi[k]);
					}
						*/
					...
					for (u64 k = numPoints8; k < mNumPoints; ++k)
					{
						...
						if(mPartyIdx==0)
						{
							ctx.bminus(T,  T, sdi[k]);
						}
						else
						{
							ctx.bplus(T, T, sdi[k]);
						}
							output(k, i, _mm_extract_epi64(T,0), tdi[k]);
						/*	
						if((i==103490 || i==0) && mPartyIdx==1)
						{
							std::cout<<"k="<<k<< " i="<<i<<"  "<<mPartyIdx<<"  "<<_mm_extract_epi64(tdi[k],0)<<std::endl;
							std::cout<<mPartyIdx<<"  gamma="<<"  "<<_mm_extract_epi64(gamma[k],0)<<std::endl;
							std::cout<<mPartyIdx<<"  sdi="<<"  "<<_mm_extract_epi64(sdi[k],0)<<std::endl;
							std::cout<<mPartyIdx<<"  "<<_mm_extract_epi64(tdi[k]&gamma[k],0)<<std::endl;
							std::cout<<mPartyIdx<<"  output=  "<<_mm_extract_epi64(T,0)<<std::endl;
						}
							*/
		...
		else
		{
			...
			for (u64 i = 0; i < mDomain; ++i)
			{
				...
				for (u64 k = 0; k < mNumPoints; ++k)
				{
					output(k, i, _mm_extract_epi64(sdi[k],0), tdi[k]);
				}
			}
		}
	}
	...
	inline u64 RegularPrimeDpf::baseOtCount() const {
		return mMultiplier.baseOtCount();
	}

	inline void RegularPrimeDpf::setBaseOts(
		span<const std::array<block, 2>> baseSendOts,
		span<const block> recvBaseOts,
		const oc::BitVector& baseChoices)
	{
	...
	inline void RegularPrimeDpf::keyGen(
		u64 domain,
		span<u64> points,
		span<u64> values,
		PRNG& prng,
